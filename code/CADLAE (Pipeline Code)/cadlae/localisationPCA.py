...
class PCALocalization:
	...
	def pca_2D(self, data, width: int = 10, height: int = 8):
		'''
	...
	def pca_3D(self, data, width: int = 10, height: int = 8, xlim_left: int = 2,
			   xlim_right: int = 7):
		'''
		...
		ax.scatter(x, y, z, c=df1["Fault"], cmap=cmap)
...
Example usage
pca_localization = PCALocalization(2)
pca_localization.fit(d_test["errors_mean"])
data_pca = pca_localization.transform(d_test["errors_mean"])
result = pca_localization.localise(5,col_names)
>> {1: 'XMV(10)', 2: 'XMEAS(9)', 3: 'XMEAS(18)', 4: 'XMEAS(20)', 5: 'XMEAS(13)'}

PCA 2D Plot
mod.pca_2D(d_test["errors_mean"])

PCA 3D Plot
mod.pca_3D(d_test["errors_mean"])
'''